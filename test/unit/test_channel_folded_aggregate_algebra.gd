...
## the channel-folded ones. Neither is a bug: one multiplies the products, the
## other adds the bonuses and multiplies once. A game that shipped on the
## first and is handed the second as "the fix" has had every stat in it
## silently rebalanced, which is why the profile decides and the default
## decides nothing.
##
## The channel-folded rules also fold in passes. Channel 0 composes, channel 1
## composes over what channel 0 produced, and so on - which is how a game says
## "this multiplies the buffed value, not the base" without every effect
## having to know about every other one.
##
## And an override behaves oppositely in the two: this engine lets the last
## one applied win, the channel-folded rules let the first eligible one claim
## the attribute. Both are defensible and they disagree, so each is written
## down.
##
...
## Put this entity on the channel-folded contracts.
func _use_channel_folded() -> void:
	asc.compatibility_profile.mode = GameplayCompatibilityProfile.Mode.CHANNEL_FOLDED
	assert_true(asc.uses_channel_folded_contracts(), "the profile took")
...
## once by the channel-folded ones. Both columns are asserted, because a
## change that made them agree would be the silent rebalance this exists to
## prevent.
##
##     [what it is, godot op, folded op, first, second, godot answer, folded answer]
...
func test_the_two_profiles_fold_the_same_authoring_differently() -> void:
	...
	for row: Array in rows:
		...
		var compatible: Array[GameplayEffectModifier] = [
			_modifier(channel_op, first_magnitude), _modifier(channel_op, second_magnitude)
		]
		_apply_each(compatible)
		assert_almost_eq(_attack(), folded, TOLERANCE, "%s: channel-folded" % described)
		checked += 1
...
func test_each_arm_of_the_channel_folded_formula_composes_as_written() -> void:
...
## `MULTIPLY` and `DIVIDE` are the channel-folded profile's own legacy
## spellings, and there they are aliases of the additive arms rather than
## compounding ones. Folding them as compound under the profile that exists to
## agree with the reference produced 2.25x where the reference produces 2.0x,
## under an operation spelled the same in both - the worst shape a difference
## can take, because nothing about the authoring looks wrong.
##
...
func test_a_compound_multiplier_still_compounds_under_the_channel_folded_profile() -> void:
	_use_channel_folded()
	_apply_each([
...
func test_the_legacy_divide_name_folds_additively_under_the_channel_folded_profile() -> void:
	_use_channel_folded()
	_apply_each([
