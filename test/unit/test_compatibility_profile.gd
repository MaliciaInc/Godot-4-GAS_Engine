...
## what the channel-folded rules do, and both have to keep working. It is data
## rather than a flag each runtime reads its own way: two runtimes asking the
...
func test_a_new_profile_is_godot_native() -> void:
	...
	assert_false(profile.is_channel_folded(), "so it does not claim the channel-folded contracts")
...
func test_a_new_component_does_not_use_channel_folded_contracts() -> void:
	...
	assert_false(
		fixture.asc.uses_channel_folded_contracts(),
		"and by default it is not answering by the channel-folded contracts"
	)
...
## own, and a shared one would mean putting a single entity onto the
## channel-folded profile moved every other entity in the scene with it.
func test_two_components_do_not_share_one_profile() -> void:
	...
	first.asc.compatibility_profile.mode = GameplayCompatibilityProfile.Mode.CHANNEL_FOLDED

	assert_true(first.asc.uses_channel_folded_contracts(), "the one that was asked, answers so")
	assert_false(
		second.asc.uses_channel_folded_contracts(),
...
## Set to the channel-folded profile, the profile and the component both say so.
func test_a_component_told_to_use_channel_folded_contracts_says_so() -> void:
	var fixture: ASCFixture = ASCFixture.create()
	add_child_autofree(fixture.owner)

	fixture.asc.compatibility_profile.mode = GameplayCompatibilityProfile.Mode.CHANNEL_FOLDED

	assert_true(
		fixture.asc.compatibility_profile.is_channel_folded(), "the profile carries the answer"
	)
	assert_true(fixture.asc.uses_channel_folded_contracts(), "and the component passes it on")
...
func test_a_component_with_no_profile_falls_back_to_godot_native() -> void:
	...
	assert_false(
		fixture.asc.uses_channel_folded_contracts(),
		"no profile is not the channel-folded contract, and not an error either"
	)
#endregion
