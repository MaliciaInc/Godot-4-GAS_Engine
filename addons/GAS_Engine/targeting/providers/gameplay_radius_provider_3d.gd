## Everything inside a circle, for as long as somebody is aiming it.
##
...
##
## The 2D mirror of this file is GameplayRadiusProvider2D, for the reason the
## overlap requests are two files each.
##
## @meta_addon: GAS_Engine
## @meta_license: GAS_Engine Community Use License 1.0
class_name GameplayRadiusProvider3D extends GameplayTargetProvider
...
func _aim() -> GameplayAbilityTargetData:
	...
	return GameplayTargetingService.overlap_3d(source_asc(), world, request)
...
func _within_range(centre: Vector3) -> bool:
	if max_range <= 0.0:
		return true
