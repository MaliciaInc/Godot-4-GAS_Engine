## What a person sees while they are aiming, in three dimensions.
##
...
## those is entirely the project's business: a decal, a ring of particles, a
## shader on the ground.
##
## The 2D mirror of this file is GameplayTargetReticle2D, for the reason the
## overlap and raycast requests are two files each: Godot's two dimensions
## share no base and no vector type.
##
...
class_name GameplayTargetReticle3D extends Node3D
...
## How big the area being aimed is, in world units. Zero for an aim that has no
...
func follow(data: GameplayAbilityTargetData) -> void:
	...
	for hit: GameplayTargetHit in data.get_all_hits():
		if hit.has_position and hit.space_kind == GameplayTargetHit.SpaceKind.THREE_D:
			global_position = hit.position_3d
			break
	_redraw()
