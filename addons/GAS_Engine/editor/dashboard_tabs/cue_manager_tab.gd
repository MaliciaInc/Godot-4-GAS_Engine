...
const SCENE_ICON: Texture2D = preload("res://addons/GAS_Engine/icons/gas_engine_cues.svg")
const CUES_ICON_PATH: String = "res://addons/GAS_Engine/icons/gas_engine_cues.svg"
...
func _load_registry() -> void:
	...
	if ResourceLoader.exists(cue_registry_path):
		...
		push_warning("GAS_Engine: Cue Registry not found at " + cue_registry_path)
...
func _on_add_mapping_pressed() -> void:
	...
	if _draft_tag == "" or _draft_scene == null:
		push_warning("GAS_Engine: Must select both a Tag and a Scene.")
		return
