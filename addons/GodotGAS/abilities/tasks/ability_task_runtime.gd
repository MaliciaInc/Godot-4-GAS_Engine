...
func register(task: GameplayAbilityTask) -> GameplayAbilityTask:
	...
	if task.owner_ability == null:
		push_error("GodotGAS: an ability task must name the ability that owns it.")
		return null
	if task.owner_ability.owner_asc != owner_asc:
		push_error("GodotGAS: an ability task cannot run on an ASC that does not own its ability.")
		return null
