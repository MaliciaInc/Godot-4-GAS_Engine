...
during F6.0.9. `after` is written by F6.4.7.

| key | before | after | what it counts |
|---|---|---|---|
| modifier_plus_ten_resources | 4 | — | Resources a person constructs to put a `+10` on an attribute: the effect, the modifier, the magnitude, and the scalable float it reads. Each is a separate "New Resource" in the inspector. |
| attribute_ref_typo_caught_before_runtime | no | — | Whether a misspelled attribute name is caught before the game runs. `GameplayAttributeRef.is_valid()` only asks whether the fields are filled in, and the asset validator has nothing to say about a name that does not exist. |
| cue_authored_without_a_script | no | — | Whether a cue with a sound and a particle can be authored as data. Today every cue is a scene whose root carries a `GameplayCueNotify` subclass somebody wrote. |
| debug_surface_outside_the_editor | no | — | Whether a running game can show ability-system state. The runtime debugger is an `EditorDebuggerPlugin`, so QA on an exported build sees nothing. |
| aoe_preview_pieces_shipped | 0 | — | What ships towards an area effect a person aims and sees before confirming: a provider that selects by radius, ground trace or placement, and anything that draws the aim. Four providers ship and all four are raycast or overlap; no reticle ships. |
| kit_grant_calls | 5 | — | Calls to put a loadout of three abilities, one effect and one attribute set onto a character, made one at a time because there is no way to say it once. |
| kit_remove_calls | 5 | — | Calls to take the same loadout off again. |
